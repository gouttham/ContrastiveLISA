...
class BASE_Transformer(ResNet):
    ...
    def forward(self, x1, x2):
        ...
        if self.with_decoder:
            dec_x1 = self._forward_transformer_decoder(x1, token1)
            dec_x2 = self._forward_transformer_decoder(x2, token2)
        else:
            dec_x1 = self._forward_simple_decoder(x1, token1)
            dec_x2 = self._forward_simple_decoder(x2, token2)

        x = torch.abs(dec_x2 - dec_x1)
        x = self.sigmoid(x)
