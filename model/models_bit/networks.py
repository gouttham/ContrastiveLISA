...
class ResNet(torch.nn.Module):
    def __init__(self, input_nc, output_nc,
        ...
        self.upsamplex2 = nn.ConvTranspose2d(256, 256, kernel_size=2, stride=2, padding=0, bias=False)
        self.upsamplex4 = nn.ConvTranspose2d(32, 32, kernel_size=4, stride=4, padding=0, bias=False)
...
class BASE_Transformer(ResNet):
    ...
    def __init__(self, input_nc, output_nc, with_pos, resnet_stages_num=5,
        ...
        # self.conv_a = nn.Conv2d(256, self.token_len, kernel_size=1, padding=0, bias=False)
        ...
        if with_pos is 'learned':
            ...
            # self.pos_embedding = nn.Parameter(torch.randn(1, self.token_len * 2, 256))
        ...
        self.f_conv = nn.Conv2d(in_channels = 32, out_channels = 3, kernel_size = 3, stride = 1, padding = 1)
        ...
        self.f2_conv_1 = nn.Sequential(
                nn.Conv2d(6, 3, kernel_size=3, stride=2), 
                nn.BatchNorm2d(3),                                     
                nn.Tanh(),)
    ...
    def forward(self, x1, x2):
        ...
        x = self.f_conv(x)
        out_attn = self.tanh(self.bn(x))

        # print(x2.shape)
        # print(out_attn.shape)
        
        x = torch.cat([skip,out_attn], 1)

        
        xx1 = self.f1_conv(x)
        

        xx2 = self.f2_conv_1(x)
        xx2 = xx2[:,:,5:-5, 5:-5]
        xx2 = self.f2_conv_2(xx2)
        xx2 = xx2[:,:,5:-5, 5:-5]
        xx2 = self.f2_conv_3(xx2)
        xx2 = xx2[:,:,6:-6, 6:-6]
        xx2 = self.f2_conv_4(xx2)
