...
NEW_PRED_DIR = "./vis_output/b1_freeze_xbd_Dahi"
NEW_TARGET_DIR = "../xbd_test/targets"
VIS_DIR = "./vis/"
...
def visualize_output(pred, target):
    ...
    colored_pred = np.zeros((pred.shape[0], pred.shape[1], 3)).astype(np.uint8)
    white_border = 255*np.ones((pred.shape[1], 1, 3))
    colored_target = np.zeros((target.shape[0], target.shape[1], 3)).astype(np.uint8)
    for i in range(len(colors)):
        colored_pred[pred == i] = colors[i]
        colored_target[target == i] = colors[i]
    return np.hstack([colored_pred, white_border, colored_target]).astype(np.uint8)


def prepare_dirs(target_dir_from_xbd, pred_dir):    
    in_filenames = glob.glob(os.path.join(pred_dir + "*_mask_0.jpg"))
    ...
    for i, file in enumerate(in_filenames):
        ...
        for p in [dp_fn, lt_fn, dt_fn]:
            check_is_present = check_is_present and os.path.isfile(p)

        if check_is_present:
            damage_mask = cv2.imread(dp_fn)
            damage_mask = np.maximum(damage_mask[:,:,0], damage_mask[:,:,1], damage_mask[:,:,2])
            # print(damage_mask.shape, np.unique(damage_mask))
            localization_mask = damage_mask.copy()
            localization_mask[localization_mask > 0] = 1
            
            target_damage_mask = cv2.imread(dt_fn)
            target_damage_mask = np.maximum(target_damage_mask[:,:,0], target_damage_mask[:,:,1], 
                                            target_damage_mask[:,:,2])
            # print(os.path.join(VIS_DIR, file, ".png"))
            cv2.imwrite(os.path.join(VIS_DIR, file + ".png"), visualize_output(damage_mask, target_damage_mask))
            # print(localization_mask.shape, np.unique(localization_mask))
            cv2.imwrite(NEW_PRED_DIR + "test_localization_" + str(i) + "_prediction.jpg", localization_mask)
            cv2.imwrite(NEW_PRED_DIR + "test_damage_" + str(i) + "_prediction.jpg", damage_mask)
...
if __name__ == '__main__':
    ...
    XviewMetrics(NEW_PRED_DIR, NEW_TARGET_DIR).compute_score(out_fp="res.json")
    del_temp_files()